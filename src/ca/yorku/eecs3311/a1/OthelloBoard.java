...
public class OthelloBoard {
	...
	char[][] board;
	/**
	 * Constructs an OthelloBoard with the specified dimension,
	 * initializing the board to its starting state with P1 and P2 tokens.
	 *
	 * @param dim The dimension of the board (typically 8)
	 */
	public OthelloBoard(int dim) {
		...
		for (int row = 0; row < this.dim; row++) {
			for (int col = 0; col < this.dim; col++) {
				this.board[row][col] = EMPTY; // Fill the board with empty tokens
			}
		}
		int mid = this.dim / 2;
		this.board[mid - 1][mid - 1] = this.board[mid][mid] = P1; // Place P1 tokens
		this.board[mid][mid - 1] = this.board[mid - 1][mid] = P2; // Place P2 tokens
	}
	/**
	 * Copy constructor to create a deep copy of another OthelloBoard instance.
	 *
	 * @param other The OthelloBoard instance to copy
	 */
	public OthelloBoard(OthelloBoard other) {
		this.dim = other.dim;
		this.board = new char[this.dim][this.dim]; // Initialize the new board
	...
	public int getDimension() {
		return this.dim; // Return the board dimension
	}
	...
	public static char otherPlayer(char player) {
		if (player == P1) return P2; // If P1, return P2
		if (player == P2) return P1; // If P2, return P1
		return EMPTY;
	}
	...
	public char get(int row, int col) {
		...
		return this.board[row][col]; // Return the token at the specified location
}
	...
	private boolean validCoordinate(int row, int col) {
		return row >= 0 && row < dim && col >= 0 && col < dim; // Check bounds
	}
	 ...
	 *         alternation
	 */
	private char alternation(int row, int col, int drow, int dcol, char player) {
		if (!validCoordinate(row + drow, col + dcol)) {
			return EMPTY; // Return EMPTY if next coordinates are invalid
		}
		...
		while (validCoordinate(row, col)) {
			...
			if (current == player && foundOpponent) {
				return player;
			}
			row += drow;
			col += dcol;
		}
		return EMPTY; // Return EMPTY if no valid alternation is found
	}
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		if (alternation(row, col, drow, dcol, player) == player) {
			int flipped = 0; // Count of flipped tokens
			row += drow;
			col += dcol;
			char opponent = otherPlayer(player);  // Get the opponent's token

			while (validCoordinate(row, col) && board[row][col] == opponent) {
				board[row][col] = player;
				flipped++;
				row += drow;
				col += dcol;
			}
			return flipped; // Return the count of flipped tokens
		}
		return -1;
	}
	 ...
	 * @return P1,P2,EMPTY
	 */
	private char hasMove(int row, int col, int drow, int dcol, char player) {
		return alternation(row, col, drow, dcol, player);
	}
				...
				if (board[row][col] == EMPTY) {
					for (int drow = -1; drow <= 1; drow++) {
						for (int dcol = -1; dcol <= 1; dcol++) {
							...
							if (hasMove(row, col, drow, dcol, P1) == P1) {
								p1HasMove = true;
							}
							if (hasMove(row, col, drow, dcol, P2) == P2) {
								p2HasMove = true;
							}
	...
	public boolean move(int row, int col, char player) {
		...
		for (int drow = -1; drow <= 1; drow++) {
			for (int dcol = -1; dcol <= 1; dcol++) {
				...
				if (hasMove(row, col, drow, dcol, player) == player) {
	...
	public boolean hasValidMove(char player) {
		// Iterate over the board and check if any valid move exists for the player
		for (int row = 0; row < this.dim; row++) {
			for (int col = 0; col < this.dim; col++) {
				if (this.board[row][col] == EMPTY) {
					// Check in all 8 directions if placing a piece would result in a valid move
					for (int drow = -1; drow <= 1; drow++) {
						for (int dcol = -1; dcol <= 1; dcol++) {
							if (drow == 0 && dcol == 0) {
	...
	public static void main(String[] args) {
		...
		for (int drow = -1; drow <= 1; drow++) {
			for (int dcol = -1; dcol <= 1; dcol++) {
				System.out.println("alternation=" + ob.alternation(4, 4, drow, dcol,P1));
			}
		}
		...
		for (int drow = -1; drow <= 1; drow++) {
			for (int dcol = -1; dcol <= 1; dcol++) {
				System.out.println("direction=(" + drow + "," + dcol + ")");
				System.out.println("hasMove at (4,4) in above direction =" + ob.hasMove(4, 4, drow, dcol,P2));
			}
		}
